...
Public Class Form1
	...
	'Log
	Public LogALL As Boolean = False
	'Font
	...
	Private Sub Set_Tims_OWON_Meter_Font()
		...
		MyApplicationVersion = FileVersionInfo.GetVersionInfo(Assembly.GetExecutingAssembly().Location).FileVersion
		Text = "Tim's OWON Meter UI   (Version " & MyApplicationVersion & ")"
		...
		AddHandler OwonB41T_Shell_Process.OutputDataReceived,
			Sub(s, evt)
				If evt.Data IsNot Nothing And Shell_Open Then
					...
					If LogALL AndAlso Form_LogALL IsNot Nothing Then

						Form_LogALL.ProcessOutputData(OwonB41T_Output)
	...
	Private Sub Button_Send_Click(sender As Object, e As EventArgs) Handles Button_Send.Click
		...
		If RichTextBox_Input.Text = "LogALL" Then
